	...
	public function login() {
		...
		if ($this->request->isPost()) {
			...
			if (!$data = $this->request->param()) {
				exit(json(['code'=>400, 'message'=>'提示：请求参数错误']));
			}

			if (!$admin = pms_admin_account_check($data['username'])) {
				exit(json_encode(['code'=>400, 'message'=>'提示：账户名不存在，请重新输入']));
			}

			if (!pms_admin_password_check($admin['id'], $data['password'])) {
				exit(json_encode(['code'=>400, 'message'=>'提示：账户名与密码不匹配，请重新输入']));
			}
			...
			exit(json_encode(['code'=>400, 'message'=>'提示：请求错误']));
		}
